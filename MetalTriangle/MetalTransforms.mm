...
float Metal::radians(const float& degrees)
{
...
simd::float4x4 Metal::scale(const float& x, const float& y, const float& z)
{
...
simd::float4x4 Metal::scale(const simd::float3& s)
{
...
simd::float4x4 Metal::translate(const simd::float3& t)
{
...
simd::float4x4 Metal::translate(const float& x,
                               const float& y,
                               const float& z)
{
    return Metal::translate((simd::float3){x,y,z});
}

#pragma mark -
#pragma mark Public - Transformations - Rotate

static float MetalRadiansOverPi(const float& degrees)
{
    return (degrees * k1Div180_f);
}

simd::float4x4 Metal::rotate(const float& angle,
                            const simd::float3& r)
{
    float a = MetalRadiansOverPi(angle);
    float c = 0.0f;
    float s = 0.0f;
                            ...
                            const float& z)
{
    ...
    return Metal::rotate(angle, r);
}

#pragma mark -
#pragma mark Public - Transformations - Perspective

simd::float4x4 Metal::perspective(const float& width,
                                 const float& height,
                                 const float& near,
                                 const float& far)
{
...
simd::float4x4 Metal::perspective_fov(const float& fovy,
                                     const float& aspect,
                                     const float& near,
                                     const float& far)
{
    float angle  = Metal::radians(0.5f * fovy);
                                     ...
                                     const float& far)
{
    ...
    return Metal::perspective_fov(fovy, aspect, near, far);
}

#pragma mark -
#pragma mark Public - Transformations - LookAt

simd::float4x4 Metal::lookAt(const simd::float3& eye,
...
simd::float4x4 Metal::lookAt(const float * const pEye,
                            ...
                            const float * const pUp)
{
    ...
    return Metal::lookAt(eye, center, up);
}

#pragma mark -
#pragma mark Public - Transformations - Orthographic

simd::float4x4 Metal::ortho2d(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 Metal::ortho2d(const simd::float3& origin,
                             const simd::float3& size)
{
    return Metal::ortho2d(origin.x, origin.y, origin.z, size.x, size.y, size.z);
}

#pragma mark -
#pragma mark Public - Transformations - Off-Center Orthographic

simd::float4x4 Metal::ortho2d_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
...
simd::float4x4 Metal::ortho2d_oc(const simd::float3& origin,
                                const simd::float3& size)
{
    return Metal::ortho2d_oc(origin.x, origin.y, origin.z, size.x, size.y, size.z);
}

#pragma mark -
#pragma mark Public - Transformations - frustum

simd::float4x4 Metal::frustum(const float& fovH,
                             const float& fovV,
                             const float& near,
                             const float& far)
{
    float width  = 1.0f / std::tan(Metal::radians(0.5f * fovH));
    float height = 1.0f / std::tan(Metal::radians(0.5f * fovV));
...
simd::float4x4 Metal::frustum(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 Metal::frustum_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
